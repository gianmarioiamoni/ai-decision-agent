...
def _ensure_string(value) -> str:
    ...
    # Prevents regex crash if LLM returns list chunks.
    if value is None:
        return ""
...
def decision_node(
    ...
    adapted_similar = [
        {
            ...
            "similarity": d.get("similarity") or 1.0,
    ...
    if adapted_similar:
        max_similarity = max(d.get("similarity", 0.0) for d in adapted_similar)
    ...
    # 🔥 CRITICAL FIX
    decision_text = _ensure_string(response.content).strip()
